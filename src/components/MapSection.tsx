        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-1 flex flex-col gap-6">
            <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-neutral-200 flex items-center gap-4">
              ...
              <span className="text-[8px] font-bold text-neutral-400">推薦城市清單</span>
            </h3>
              ...
              {featuredCities.map((city) => (
                <div 
                  ...
                  className={`group cursor-pointer flex justify-between items-center py-6 border-b border-neutral-500/30 transition-all duration-300 ${
                    selectedCity === city.name ? 'px-4 bg-neutral-700' : 'hover:px-4 hover:bg-neutral-500/10'
                  }`}
                >
                  <div>
                    <div className="flex items-baseline gap-3">
                      <h4 className={`font-black uppercase text-2xl tracking-tight transition-colors ${
                        selectedCity === city.name ? 'text-white' : 'text-neutral-400 group-hover:text-neutral-100'
                      }`}>
                        {city.name}
                      </h4>
                      <span className="text-xs font-bold text-neutral-500">{city.cn}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-6">
                    <div className={`text-2xl font-black italic tracking-tighter ${
                      selectedCity === city.name ? 'text-white' : 'text-neutral-500 group-hover:text-neutral-300'
                    }`}>
                      {city.temp}°
                    </div>
                    <ArrowRight className={`w-4 h-4 transition-all duration-300 ${
                      selectedCity === city.name ? 'text-white' : 'opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 text-neutral-500'
                    }`} />
                  </div>
                </div>
              ))}
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            ...
            <div className="flex justify-between items-center text-[8px] font-bold text-neutral-500 uppercase tracking-[0.2em]">
              ...
              <div className="flex gap-4">
                <span className="cursor-pointer hover:text-white transition-colors uppercase">Layers</span>
                <span className="cursor-pointer hover:text-white transition-colors uppercase">Zoom</span>
              </div>
            </div>
          </div>
        </div>
